...
def variable_plot(source_airbnb, source_booking, source_both):
    ...
    p=figure(x_range=[],y_range=(0, 100), plot_width=1300, plot_height=300)
    ...
    p.vbar(x=dodge('x', 0.25, range=p.x_range), top='top', width=0.2, source=source_both,
       legend=value("Soma dos dados (por agrupamento)"), color="purple")
...
def button1_callback():
    ...
    df = None
    if df is None and ub.necessidade_de_carregamento(r, inps):
...
kclusters = Select(title="Quantidade de clusters gerados:",
...
layout = column(div, row(inputs, column(divMapa, plot), width=1000),
                column(divVariable, p_airbnb), column(divCorr, p_corr),
