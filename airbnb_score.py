...
def search(config, city, args):
	try:
		...
		sql = """SELECT distinct(room_id) from room where city = %s
				and deleted = 0 and room_id >= 41564161
				order by room_id""" #and comodities is null
				#and price is null
				#and overall_satisfaction is null
				...
				for i in range(config.ATTEMPTS_TO_FIND_PAGE):
					try:
						...
						try:
							...
							driver.find_element_by_xpath('/html/body/div[6]/div/div/div[1]/section/footer/div[2]').click()
						time.sleep(3)  
						if args.comments:              
...
def get_reviews(config, driver, city, room_id):
	try:
		...
		try:
			driver.find_element_by_xpath('//main/div/div/div[4]/div/div/div[2]/div[4]/a').click()
		except selenium.common.exceptions.NoSuchElementException:
	...
	while True:
		try:
			element = driver.find_element_by_xpath('/html/body/div[13]/section/div/div/div[3]/div/div/section/div/div[2]/div/div[' + str(i) + ']')
			driver.execute_script("arguments[0].scrollIntoView();", element)
			print(i)
			time.sleep(10)
			#rev = reviews.find_elements_by_xpath('//div[@class="_1gjypya"]')
			i = i + 7
		except selenium.common.exceptions.NoSuchElementException:
			for j in range(i + 7, -1, 2):
				try:
					element = driver.find_element_by_xpath('/html/body/div[13]/section/div/div/div[3]/div/div/section/div/div[2]/div/div[' + str(j) + ']')
					driver.execute_script("arguments[0].scrollIntoView();", element)
					print(j)
					break
				except:
