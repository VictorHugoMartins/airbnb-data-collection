...
CITY_OPTIONS = ['Diamantina', 'Tiradentes', 'Ouro Preto']
...
def filter_data(df, inps):
    ...
    if inps.region.value != 'ALL':
        query = query + ' & region == "' + inps.region.value + '"'
    if inps.room_type.value != 'ALL':
        query = query + ' & room_type == "' + inps.room_type.value + '"'
    if inps.category.value != 'ALL':
        query = query + ' & category == "' + inps.category.value + '"'
    if inps.site.value != 'ALL':
        query = query + ' & site == "' + inps.site.value + '"'
    if inps.consulta.value != '':
        query = query + ' & ' + inps.consulta.value

    clusters = df['cluster'].unique().tolist()
    if int(inps.cluster.value - 1) in clusters:
        query = query + ' cluster == ' + str(inps.cluster.value - 1)

    print(df.columns)
    # query = 'table != "Airbnb and Booking"'
    # print("cidades distintas:", df['cidade'].unique())
    # print("tabelas distintas:", df['table'].unique())
    print("AO QUERY", query)
    df = df.query(query)
    print("o df filtrado", df)
    return df
...
def get_x_range(c):
    if c == 'region':
        return list(REGION_OPTIONS)
    elif c == 'room_type':
        return list(ROOM_TYPE_OPTIONS)
    elif c == 'category':
        return list(CATEGORY_OPTIONS)
    elif c == 'comodities':
        return list(COMODITIES_OPTIONS)
    elif c == 'bathroom':
        return list(BATHROOM_OPTIONS)
    elif c == 'site':
        return list(SITE_OPTIONS)

# Get y range for variable plot


def get_y_range(n):
    if n == 'overall_satisfaction':
        return (0, 6)
    elif n == 'price_pc':
        return (0, 700)
    elif n == 'quantidade':
...
def dataframe_for_vbar(df, site, categorical_column, value_column):
    ...
    if site != 'Airbnb and Booking':
        df = df[df.site == site]
    if categorical_column in ['region', 'room_type', 'category', 'site']:
        categorical_values = []
        if categorical_column == 'region':
            categorical_types = REGION_OPTIONS
        elif categorical_column == 'room_type':
            categorical_types = ROOM_TYPE_OPTIONS
        elif categorical_column == 'category':
            categorical_types = CATEGORY_OPTIONS
        elif categorical_column == 'site':
            categorical_types = SITE_OPTIONS
        ...
        if categorical_column == 'comodities':
            return count_columns_dataframe(df, COMODITIES_OPTIONS, value_column)
        elif categorical_column == 'bathroom':
            return count_columns_dataframe(df, BATHROOM_OPTIONS, value_column)
...
def get_corr_columns(df, site):
    ...
    df_corr = df
    if site != 'Airbnb and Booking':
        df_corr = df_corr[df_corr.site == site]
    try:
